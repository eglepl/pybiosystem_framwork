...
classdef BioSystem < handle
    ...
    methods
        ...
        function new_compositor = AddCompositor(self, varargin)
            if nargin() == 2  
               % we were given a Compositor as the input
                new_compositor = varargin{1};
            elseif nargin() == 3  
                % we were given a name and an initial value
        ...
        function new_constant = AddConstant(self, varargin)
            if nargin() == 2 
                % we were given a Const as the input
                new_constant = varargin{1};
            elseif nargin() == 3  
                % we were given a name and a value
                new_constant = Const(varargin{1}, varargin{2});
            else
                error('Incorrect call to AddConstant')
            end
        ...
        function [] = determine_rates(self)
            if ~self.rates_determined
                ...
                for k = 1:length(self.compositors)
                    ...
                    symexpr = subs(sym(self.compositors(k).rate), ...
                                constant_syms, ...
                                constant_vals);
        ...
        function [] = ChangeConstantValue(self, name, value)
            ...
            self.reset_rates() 
            % calculated rates use the numerical value of
        ...
        % If we are given an extra parameters, they are passed to ode23s
        ...
        function [T, Y] = run(self, tspan, varargin)
			...
			nonneg = odeset('NonNegative', true);  % ensure values are positive
            if isempty(varargin)
				
                [ T, Y ] = ode23s(@self.sys_ode, tspan, y0, nonneg);
            else
                [ T, Y ] = ode23s(@self.sys_ode, tspan, y0, odeset(nonneg, varargin{:}));
            end
        ...
        function dy = sys_ode(self, t, y)
            ...
            % hack to unpack the vector. optimize?
            for i = 1:length(self.compositors)
        ...
        function [T, Y] = run_pulses(self, pulse_series)
            ...
            prev_start = 0;
            prev_end = 0;
            % last pulse is not actually simulated
